    ...
    if patient:
        if patient.customer:
                ...
                for patient_appointment in patient_appointments:
                    ...
                    if patient_appointment_obj.procedure_template:
                        if frappe.db.get_value("Clinical Procedure Template",
                                               patient_appointment_obj.procedure_template, "is_billable") == 1:
                            item_to_invoice.append({'reference_type': 'Patient Appointment',
                                                    'reference_name': patient_appointment_obj.name,
                                                    'service': patient_appointment_obj.procedure_template})
                    else:
                        ...
                        if not practitioner_exist_in_list:
                            valid_till = patient_appointment_obj.appointment_date + datetime.timedelta(
                                days=int(valid_days))
                            visits = 0
                            validity_exist = validity_exists(patient_appointment_obj.practitioner,
                        ...
                        if not skip_invoice:
                            ...
                            if patient_appointment_obj.practitioner:
                                ...
                                income_account = get_income_account(patient_appointment_obj.practitioner,
            ...
            encounters = frappe.get_list("Patient Encounter",
                                         {'patient': patient.name, 'invoiced': False, 'docstatus': 1})
            if encounters:
                for encounter in encounters:
                    ...
                    if not encounter_obj.appointment:
                        ...
                        item_to_invoice.append(
                            {'reference_type': 'Patient Encounter', 'reference_name': encounter_obj.name,
                             'service': service_item, 'rate': practitioner_charge,
                             'income_account': income_account})
                ...
                for lab_test in lab_tests:
                    ...
                    if frappe.db.get_value("Lab Test Template", lab_test_obj.template, "is_billable") == 1:
                        item_to_invoice.append({'reference_type': 'Lab Test', 'reference_name': lab_test_obj.name,
                                                'service': frappe.db.get_value("Lab Test Template",
                ...
                for procedure in procedures:
                    ...
                    if not procedure_obj.appointment:
                        if procedure_obj.procedure_template and (
                                frappe.db.get_value("Clinical Procedure Template", procedure_obj.procedure_template,
                                                    "is_billable") == 1):
                            item_to_invoice.append(
                                {'reference_type': 'Clinical Procedure', 'reference_name': procedure_obj.name,
                                 'service': frappe.db.get_value("Clinical Procedure Template",
            ...
            if procedure_rxs:
                for procedure_rx in procedure_rxs:
                    ...
                    if frappe.db.get_value("Clinical Procedure Template", rx_obj.procedure, "is_billable") == 1:
                        item_to_invoice.append(
                            {'reference_type': 'Procedure Prescription', 'reference_name': rx_obj.name,
            ...
            procedures = frappe.get_list("Clinical Procedure",
                                         {'patient': patient.name, 'invoice_separately_as_consumables': True,
            ...
            if procedures:
                ...
                for procedure in procedures:
                    ...
                    item_to_invoice.append(
                        {'reference_type': 'Clinical Procedure', 'reference_name': procedure_obj.name,
                         'service': service_item, 'rate': procedure_obj.consumable_total_amount,
                         'description': procedure_obj.consumption_details})
            ...
            if inpatient_services:
                for inpatient_service in inpatient_services:
                    ...
                    service_unit_type = frappe.get_doc("Healthcare Service Unit Type",
                    ...
                    if service_unit_type and service_unit_type.is_billable == 1:
                        ...
                        if hours_occupied > 0:
                            ...
                            if qty <= 0:
                                qty = 0.5
                        item_to_invoice.append(
                            {'reference_type': 'Inpatient Occupancy', 'reference_name': inpatient_occupancy.name,
...
def validate_invoiced_on_submit(item):
    if item.reference_dt == 'Clinical Procedure' and get_healthcare_service_item(
            'clinical_procedure_consumable_item') == item.item_code:
...
def manage_fee_validity(appointment_name, method, ref_invoice=None):
    ...
    if validity_exist:
        ...
        fee_validity = create_fee_validity(appointment_doc.practitioner, appointment_doc.patient,
                                           appointment_doc.appointment_date, ref_invoice)
        visited = fee_validity.visited
            ...
            for drug_line in encounter.drug_prescription:
                if drug_line.drug_code:
                    ...
                    item_to_invoice.append({'reference_type': 'Drug Prescription', 'reference_name': drug_line.name,
                                            'drug_code': drug_line.drug_code, 'quantity': qty,
...
def get_children(doctype, parent, company, is_root=False):
    ...
    if is_root:
        ...
        fields += ['service_unit_type', 'allow_appointments', 'inpatient_occupancy',
                   'status'] if doctype == 'Healthcare Service Unit' else []
