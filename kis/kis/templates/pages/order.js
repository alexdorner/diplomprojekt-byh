...
frappe.ready(function(){
	...
	function apply_loyalty_points() {
		...
		if (loyalty_points) {
			frappe.call({
				method: "kis.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
				...
				callback: function(r) {
					if (r) {
						...
						if (doc_info.grand_total && doc_info.grand_total < loyalty_amount) {
							...
							payment_button.href = "/api/method/kis.accounts.doctype.payment_request.payment_request.make_payment_request?dn="+doc_info.doctype_name+"&dt="+doc_info.doctype+"&loyalty_points="+loyalty_points+"&submit_doc=1&order_type=Shopping Cart";
						}
