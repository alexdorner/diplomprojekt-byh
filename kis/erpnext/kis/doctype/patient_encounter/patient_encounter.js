...
frappe.ui.form.on('Patient Encounter', {
			...
			if (frm.doc.drug_prescription && frm.doc.inpatient_record && frm.doc.inpatient_status === "Admitted") {
				frm.add_custom_button(__('Inpatient Medication Order'), function() {
					frappe.model.open_mapped_doc({
						method: 'erpnext.kis.doctype.patient_encounter.patient_encounter.make_ip_medication_order',
						frm: frm
					});
				}, 'Create');
			}
	...
	set_patient_info: function(frm) {
		if (frm.doc.patient) {
			frappe.call({
				method: 'erpnext.kis.doctype.patient.patient.get_patient_detail',
				args: {
...
var schedule_inpatient = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action : function() {
			...
			frappe.call({
				method: 'erpnext.kis.doctype.inpatient_record.inpatient_record.schedule_inpatient',
				args: {
					args: args
				},
				callback: function(data) {
...
var schedule_discharge = function(frm) {
	var dialog = new frappe.ui.Dialog ({
		...
		primary_action : function() {
			...
			frappe.call ({
				method: 'erpnext.kis.doctype.inpatient_record.inpatient_record.schedule_discharge',
				args: {args},
				callback: function(data) {
					if(!data.exc){
						frm.reload_doc();
