...
let change_template_code = function(doc) {
	let d = new frappe.ui.Dialog({
		...
		primary_action: function() {
			...
			if (values) {
				frappe.call({
					'method': 'erpnext.kis.doctype.clinical_procedure_template.clinical_procedure_template.change_item_code_from_template',
...
frappe.ui.form.on('Clinical Procedure Item', {
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (d.item_code) {
			...
			return frappe.call({
				method: 'erpnext.kis.doctype.clinical_procedure_template.clinical_procedure_template.get_item_details',
				args: {args: args},
				callback: function(r) {
					if (r.message) {
						let d = locals[cdt][cdn];
...
frappe.tour['Clinical Procedure Template'] = [
	...
	},
	{
		...
		description: __('Check this if the Clinical Procedure utilises consumables. Click ') + "<a href='https://docs.erpnext.com/docs/user/manual/en/kis/clinical_procedure_template#22-manage-procedure-consumables' target='_blank'>here</a>" + __(' to know more')
